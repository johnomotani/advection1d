...
#include "chebyshevfft_r2r.hxx"

ChebyshevFFT_r2r::ChebyshevFFT_r2r(const Parameters &parameters)
...
double ChebyshevFFT_r2r::v(const double t, const int i) const {
  return 0.1;
  // return 0.1 + 0.05 * sin(2.0 * pi * z[i] / L);
}

void ChebyshevFFT_r2r::applyBoundary(const double t, Array &f) const {
  switch (bc) {
  case BC::periodic:
    f[0] = f[Nz - 1];
    break;
